...
ADAETTE <- ADAETTE %>%
  ...
  teal.widgets::formatters_var_relabel(is_event = "Is an Event") %>%
  teal.widgets::formatters_var_relabel(n_events = "Number of Events")
ADAETTE_AE <-
...
ADEX_labels <- teal.widgets::formatters_var_labels(ADEX, fill = FALSE)
...
teal.widgets::formatters_var_labels(ADEX) <- ADEX_labels
...
ADVS <- synthetic_cdisc_data("latest")$advs %>%
  ...
  teal.widgets::formatters_var_relabel(ONTRTFL = "On Treatment Record Flag") %>%
