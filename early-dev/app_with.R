...
##
{
  adsl_labels <- teal.data::col_labels(ADSL) |> with(data, expr = _)
  fact_vars_asl <- names(Filter(isTRUE, sapply(ADSL, is.factor))) |> with(data, expr = _)

  date_vars_asl <- names(ADSL)[vapply(ADSL, function(x) inherits(x, c("Date", "POSIXct", "POSIXlt")), logical(1))] |> with(data, expr = _)
  demog_vars_asl <- names(ADSL)[!(names(ADSL) %in% c("USUBJID", "STUDYID", date_vars_asl))] |> with(data, expr = _)
}


## Reusable Configuration For Modules
{
  arm_vars <- c("ARMCD", "ARM", "ACTARMCD", "ACTARM", "EOSSTT") |> with(data, expr = _)
  aeflag_vars <- c("RELFL", "CTC35FL", "SERFL", "RELSERFL") |> with(data, expr = _)
  facet_vars <- c("SEX", "BMRKR2", "RACE", "STRATA1", "STRATA2") |> with(data, expr = _)
  ds_vars <- c("EOSSTT", "DCSREAS") |> with(data, expr = _)
